...
namespace mpc_utils
{
  ...
  class Piece 
  {
    ...
    public:
      ...
      Piece(double dur, const CoefficientMat<D> &cMat)
          : duration(dur), coeffMat(cMat), singular(1.0) {}

      Piece(double dur, const CoefficientMat<D> &cMat, double sgl)
          : duration(dur), coeffMat(cMat), singular(sgl) {}
      ...
      {
        Eigen::Matrix<double, D, 1> vel = getVel(t);
        return atan2(singular*vel.y(), singular*vel.x());
      }
      ...
      inline double getLonVel(const double &t) const 
      {
        Eigen::Matrix<double, D, 1> vel = getVel(t);
        return singular * vel.norm();
      }
      ...
      inline double getLonAcc(const double &t) const 
      {
        Eigen::Matrix<double, D, 1> acc = getAcc(t);
        double yaw = getAngle(t);

        return acc.dot(Eigen::Vector2d(cos(yaw), sin(yaw)));
      }

      inline double getCur(const double &t) const 
      {
        Eigen::Matrix<double, D, 1> acc = getAcc(t);
        Eigen::Matrix<double, D, 1> vel = getVel(t);
        double cur = acc.y() * vel.x() - acc.x() * vel.y();
        double v_norm = vel.norm();
        if (v_norm < 1.0e-6)
          return 0.0;
        else
          return singular * cur / (v_norm * v_norm * v_norm);
      }
      inline double getOmega(const double& t, double wheelbase = 0.6) const{

            Eigen::Matrix<double, D, 1> vel = getVel(t);
            Eigen::Matrix<double, D, 1> acc = getAcc(t);
            Eigen::Matrix<double, D, 1> jerk= getJer(t);
  ...
  public:
    ...
    Trajectory(const std::vector<double> &durs,
              const std::vector<CoefficientMat<D>> &cMats,
              const std::vector<double> & sgls) 
    {
      int N = std::min(durs.size(), cMats.size());
      pieces.reserve(N);
      for (int i = 0; i < N; i++) {
        pieces.emplace_back(durs[i], cMats[i], sgls[i]);
      }
    }
    ...
    inline double getLonVel(double t) const 
    {
      int pieceIdx = locatePieceIdx(t);
      return pieces[pieceIdx].getLonVel(t);
    }
    ...
    {
      int pieceIdx = locatePieceIdx(t);
      return pieces[pieceIdx].getLonAcc(t);
    }

    inline double getCur(double t) const 
    {
      int pieceIdx = locatePieceIdx(t);
      return pieces[pieceIdx].getCur(t);
    }
    inline double getPhi(double t, double wheelbase = 0.6) const{
        double c = getCur(t);
        return atan(c * wheelbase);
    }

    inline double getOmega(double t, double wheelbase = 0.6) const{
      int pieceIdx = locatePieceIdx(t);
      return pieces[pieceIdx].getOmega(t, wheelbase);
    }
  ...
  class MincoTraj 
  {
    ...
    typedef Trajectory<2> FlatTraj2d; 
}