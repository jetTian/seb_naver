...
class map_generator(object):
    ...
    def init_params(self):
        ...
        self.resolution = rospy.get_param("map/resolution", default=0.15)
        ...
        self.rate = rospy.get_param("sensing/rate", default=2.0)
    ...
    def add_layer(self, size, position, res=None):
        ''' size: [x,y]
            position: [x,y,z] --- center position
        '''        
        if res is None:
            res = self.resolution
        x_low = math.floor((position[0] - size[0] / 2) / res) * res + 0.5 * res
        x_high = math.floor((position[0] + size[0] / 2) / res) * res  + 0.5 * res
        y_low = math.floor((position[1] - size[1] / 2) / res) * res  + 0.5 * res
        y_high = math.floor((position[1] + size[1] / 2) / res) * res  + 0.5 * res
        z = position[2]
    ...
    def make_map(self):
        ...
        obs_height = 3.0
        pos_height = obs_height - 1.0

        self.add_cylinder([0.5, obs_height], [-8, 10, pos_height])

        self.add_cylinder([0.5, obs_height], [0.8, 8.0, pos_height])

        self.add_cylinder([0.5, obs_height], [8.0, 9.0, pos_height])

        self.add_cylinder([0.5, obs_height], [-5.0, 3.0, pos_height])

        self.add_cylinder([0.5, obs_height], [6.0, 1.0, pos_height])

        self.add_cylinder([0.5, obs_height], [-6.0, -6.0, pos_height])

        self.add_cylinder([0.5, obs_height], [1.0, -8.0, pos_height])

        self.add_cylinder([0.5, obs_height], [8.0,-3.0, pos_height])

        self.add_box([1.0, 1.0, obs_height], [-12.0, 10.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [3.0, 10.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [13.0, 9.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [-10.0, 3.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [3.0, 3.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [10.0, 3.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [-12.0, -3.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [-6.0, -1.0, pos_height])
        self.add_box([1.0, 2.0, obs_height], [0.0, -1.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [13.0, -1.2, pos_height])
        self.add_box([1.0, 1.0, obs_height], [-6.5, -10.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [6.5, -7.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [-3.0, 6.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [-3.0, -6.0, pos_height])
        self.add_box([1.0, 1.0, obs_height], [3.0, -3.0, pos_height])

        self.add_layer([100.0, 100.0], [0.0, 0.0, 0.0])
